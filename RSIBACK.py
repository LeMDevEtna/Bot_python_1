...
candlesticks = client.get_historical_klines("ETHUSDT", Client.KLINE_INTERVAL_30MINUTE, "28 AUG, 2021", "3 SEP, 2021")
walet = 1000
start = 0
end = 0
closes = []
...
def on_message():
    ...
    if len(candlesticks) > RSI_PERIOD:
        ...
        for i in range(len(rsi)):
            ...
            if rsi[i] > 76.6  and float(candlesticks[i][4]) > float(tmp) and float(tmp) != 0: 
